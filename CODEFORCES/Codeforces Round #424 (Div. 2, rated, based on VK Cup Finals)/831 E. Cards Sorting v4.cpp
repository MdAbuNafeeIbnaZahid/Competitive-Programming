...
LL getAddee(LL last, LL now)
{
    if ( last < now )
    {
        return (now-last)-( myBit.read(now)-myBit.read(last) );
    }
    else
    {
        LL ret = (n-last)+now;
        ret -= (myBit.read(n) - myBit.read(last) );
        ret -= (myBit.read(now) );
        return ret;
    }
}
...
//    DBG(n);
    ...
    FOR(a, 1, SIZE)
    {
        while( setAr[a].size() )
        {
            auto it = setAr[a].lower_bound( lastRemoved );
            if ( it == setAr[a].end() )
            {
                it = setAr[a].begin();
            }
            LL toRemove = *it;
            setAr[a].erase( it );
            LL addee = getAddee(lastRemoved, toRemove);
            ans += addee;
            lastRemoved = toRemove;
            myBit.update(toRemove, 1);
        }
    }
    cout << ans;
    return 0;
