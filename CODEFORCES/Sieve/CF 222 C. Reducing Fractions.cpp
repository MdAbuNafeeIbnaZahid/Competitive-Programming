...
// lpfVec[0] = 0
// lpfVec[1] = 0
vector<int> getLpfVec( long long upto )
{
    long long a, b, c, d, e;
    vector<int > lpfVec(upto+1, 0);
    ...
    for (a = 2; a <= upto; a++)
    {
        ...
        for (b = 0; b < primeVec.size(); b++)
        {
            long long curPrime = primeVec[b];
            long long nextComposite = a*curPrime;
            if ( nextComposite > upto || curPrime>lpfVec[a] )
            {
                break;
            }
            lpfVec[ nextComposite ] = curPrime;
        }
    }
    return lpfVec;
}

VL getPrimeCntVec(VL numVec, const VL &lpfVec)
{
    VL primeCntVec(SIZE+9, 0);
    for (auto it : numVec)
    {
        while(1)
        {
...
void minimizeVec(VL &A, VL &B)
{
    ...
    FOR(a,0,len)
    {
        LL minVal = min( A[a], B[a] );
        A[a] -= minVal;
        B[a] -= minVal;
    }
}

VL getAnsVec(VL &productVec, VL initVec, const VL &lpfVec)
{
    LL a, b, c, d, e;
    VL ansVec;
    for(auto it : initVec)
    {
        ...
        ansVec.push_back( curPush );
    }
    return ansVec;
}


void solve(VL A, VL B)
{
    DBG( "in solve" );
    VL lpfVec = getLpfVec( SIZE+9 );
    DBG(lpfVec);
    VL primeCntA = getPrimeCntVec(A, lpfVec);
    DBG( primeCntA.size() );
    VL primeCntB = getPrimeCntVec(B, lpfVec);
    DBG( primeCntB.size() );
    DBG( primeCntA );
    DBG( primeCntB );
    minimizeVec(primeCntA, primeCntB);
    DBG( "vectors minimized" );
    DBG( primeCntA );
    DBG( primeCntB );
    VL ansVecA = getAnsVec(primeCntA, A, lpfVec);
    VL ansVecB = getAnsVec(primeCntB, B, lpfVec);
    cout << ansVecA.size() << " " << ansVecB.size() << endl;
    for (auto it : ansVecA)
    {
        printf("%d ", it);
    }
