...
/******   END OF HEADER *********/
#define SIZE 5000009LL


VL getCumSumVec( VL numVec )
{
    LL a, b, c, d, e;
    LL len = numVec.size();
    VL cumSumVec(len, 0);
    if (len > 0)
    {
        cumSumVec[0] = numVec[0];
    }
    FOR(a,1,len)
    {
        cumSumVec[a] = cumSumVec[a-1] + numVec[a];
    }
    return cumSumVec;
}

VL getPrimeFactCntVec( LL upto )
{
    LL a, b, c, d, e;
    VL primeFactCntVec(upto+1, 0);
    VL lpfVec(upto+1, 0);
    VL primeVec;
    FOR(a,2,upto+1)
    {
        ...
        for (auto it : primeVec)
        {
            LL nextCompo = it*a;
            if ( it>lpfVec[a] || nextCompo >upto )
            {
                break;
            }
            lpfVec[nextCompo] = it;
            primeFactCntVec[nextCompo] = primeFactCntVec[a]+1;
        }
    }
    return primeFactCntVec;
}
...
//    freopen("input.txt", "r", stdin);
    ...
    LL t, u, v;
    LL a, b, c, d, e;
    VL primeFactCntVec = getPrimeFactCntVec(SIZE);
    VL cumSumPFCntVec = getCumSumVec( primeFactCntVec );
    cin >> t;
    FOR(a,1,1+t)
    {
        LL u, v;
        iLL2(u, v);
        LL ans = cumSumPFCntVec[u] - cumSumPFCntVec[v];
        printf("%lld\n", ans);
    }
    return 0;
}
