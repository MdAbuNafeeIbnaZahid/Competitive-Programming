...
map< set<LL>, LL > grundyMap;
map<LL, set<LL> > powMap;

LL n, aAr[SIZE];

LL getGrundyVal( set<LL> setLL )
{
    LL a, b, c, d, e, f;
//    cout << "in getGrundyVal " << endl;
//    printSetLL( setLL );
    ...
    if ( grundyMap.find( setLL ) == grundyMap.end() )
    {
        LL maxVal = *setLL.rbegin();
        set<LL> lowerValSet;
        FOR(a,1,maxVal+1)
        {
            set<LL> lowerParam;
            for (auto it : setLL)
            {
                ...
                {
                    lowerParam.insert( it );
                }
            }
            LL lowerVal = getGrundyVal( lowerParam );
            lowerValSet.insert( lowerVal );
        }
        FOR(a,0,INT_MAX)
        {
            if ( lowerValSet.find(a) == lowerValSet.end() )
            {
                grundyMap[setLL] = a;
                break;
            }
...
{
    LL a, b, c, d, e;
    cout << "printing powMap " << endl;
    for (auto it : powMap)
    {
        DBG( it.first );
        printSetLL( it.second );
    }
}
...
//        DBG( it.first );
        ...
        LL grundyVal = getGrundyVal( it.second );
//        DBG( grundyVal );
        xorVal ^= grundyVal;
    }
