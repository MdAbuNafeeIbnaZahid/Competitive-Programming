...
#define SIZE 109
LL n;
LL pointAr[SIZE][SIZE];
set<LL> goodSet;

LL getDot(LL a, LL b, LL c)
{
    LL i, j, k;
    LL ret = 0;
    FOR(i,1,1+5)
    {
        ret += ( pointAr[b][i]-pointAr[a][i] ) * ( pointAr[c][i] - pointAr[a][i] );
    }
    return ret;
}
...
//    freopen("input.txt", "r", stdin);
    ...
    FOR(a,1,1+n)
    {
        cin >> pointAr[a][1] >> pointAr[a][2] >> pointAr[a][3] >> pointAr[a][4]
                >> pointAr[a][5];
    }
    FOR(a,1,1+n)
    {
