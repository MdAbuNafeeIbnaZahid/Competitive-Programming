...
//LL getCalc()
vector<LL> oneDown(vector<LL> giVec)
{
    LL a, b, c, d, e, si = 1;
    vector<LL> ret;
    FOR(a,1,giVec.size()-1)
    {
        ret.PB( giVec[a-1] + si * giVec[a] );
        si *= -1;
    }
    return ret;
}

LL getAns(vector<LL> giVec )
{
    LL a, b, c, d, e, ans = 0;
    map<LL, LL> smartMap = zeroTwo(giVec.size());
    FOR(a,1,giVec.size())
    {
        ans = ( ans + ( smartMap[a] * giVec[a-1] ) % MOD ) % MOD;
    }
    return (ans+MOD)%MOD;
}
