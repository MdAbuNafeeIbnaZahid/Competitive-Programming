...
LL mulCntAr[SIZE], tempKSumAr[SIZE];
LL ans;
void sivAndMob()
{
    LL a, b, c, d, e, f;
...
//    }
    ...
    ROF(a,SIZE-1,1)
    {
        LL ansAddee = 0;
        LL gcd = a;
        tempKSumAr[ gcd ] = kAr[ mulCntAr[gcd] ];
        for (b = gcd<<1LL; b < SIZE; b += gcd)
        {
//            tempKSum = ( ( kAr[ mulCntAr[b*gcd] ] * mobiusAr[b] + MOD )%MOD + tempKSum ) % MOD;
            tempKSumAr[ gcd ] = (tempKSumAr[gcd] - tempKSumAr[ b ] + MOD) % MOD;
        }

        ansAddee = ( tempKSumAr[gcd] * gcd ) % MOD;
