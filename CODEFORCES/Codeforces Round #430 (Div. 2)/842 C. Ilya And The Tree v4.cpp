...
#define SIZE 200009
LL n, aAr[SIZE];
LL ansAr[SIZE], gcdAr[SIZE], colorAr[SIZE], parentAr[SIZE];
vector<LL> gr[SIZE];
void dfs1(LL u, set<LL> oneSkippedIn)
{
    LL a, b, c, d, e;
    if ( colorAr[u] )
    {
...
//    cout << "in dfs1" << endl;
//    DBG(u);
//    cout << "oneSkipped In" << endl;
//    printSetLL( oneSkippedIn );

    gcdAr[u] = getGcd( aAr[u], gcdAr[ parentAr[u] ] );
    ansAr[u] = gcdAr[u];
    set<LL> oneSkippedOut; // = oneSkippedIn;
    for (auto it : oneSkippedIn)
    {
        LL toIns =  getGcd( aAr[u], it );
        oneSkippedOut.insert( toIns );
        ansAr[u] = max( toIns, ansAr[u] );
    }
    oneSkippedOut.insert( gcdAr[ parentAr[u] ] );
    ansAr[u] = max( gcdAr[ parentAr[u] ], ansAr[u] );
