...
//    freopen("input.txt", "r", stdin);
    ...
    FOR(a,0,vecPair.size())
    {
        makePair( a, vecPair[a].second );
    }
    set<LL> parentSet;
    FOR(a,0,n)
    {
        parentSet.insert( getParent(a) );
        vecAr[ getParent(a) ].push_back( a );
    }
    ...
    FOR(a,0,n)
    {
        if ( vecAr[a].size() )
        {
            printf("%lld ", (long long)vecAr[a].size() );
            for (auto it : vecAr[a])
            {
                printf("%lld ", it+1);
            }
