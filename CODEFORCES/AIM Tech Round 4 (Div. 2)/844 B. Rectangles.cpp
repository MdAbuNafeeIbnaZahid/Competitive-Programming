...
LL n, m;
LL grid[SIZE][SIZE];
LL pow2[SIZE], rowSum[SIZE], colSum[SIZE];
LL ans;

LL getSetCnt(LL num)
{
    return (1LL<<num)-1LL-num;
}
...
//    freopen("input.txt", "r", stdin);
    ...
    FOR(a,1,1+n) // rows
    {
        ans += getSetCnt( rowSum[a] );
        ans += getSetCnt( m - rowSum[a] );
    }

    FOR(a,1,1+m) // cols
    {
        ans += getSetCnt( colSum[a] );
        ans += getSetCnt( n - colSum[a] );
    }
