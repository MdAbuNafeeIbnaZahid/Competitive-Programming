...
LL maxDifPart, maxDif;

LL getValSum(LL idx1, LL idx2)
{
    LL len = valIdxVec.size();
    assert( idx1>= 0 && idx1<len );
    assert( idx2>=0 && idx2<len );
    return valIdxVec[idx1].first + valIdxVec[idx2].first;
}
...
LL getMaxDif()
{
    SORT(valIdxVec);
    LL len = valIdxVec.size();
    LL ret = getValSum( len-1, len-2 ) - getValSum(0, 1);
    return ret;
}

void printAns()
{
    dbg2(maxDif, maxDifPart);
    cout << min(maxDif, maxDifPart) << endl;
    bool isPart = ( maxDifPart < maxDif );
    FOR(a,0,n)
    {
        if ( a==minIdx && isPart )
        {
...
//    freopen("input.txt", "r", stdin);
    ...
    {
        iLL( aAr[a] );
        valIdxVec.push_back( PairLL(aAr[a], a) );
    }

    SORT(valIdxVec);
    minIdx = valIdxVec[0].second;
