...
void makeCylceVec()
{
    FOR(a,1,1+n)
    {
        ...
        if ( cycle.size() > 0 )
        {
            cycleVec.push_back( cycle );
        }
    }
}

LL getCycleIdx(LL siz)
{
    FOR(a,0,cycleVec.size())
    {
        if ( cycleVec[a].size() == siz )
        {
            return a;
        }
    }
...
void printEdge(const VL &headVec, const VL &childVec)
{
    LL headSize = headVec.size();
    LL childSize = childVec.size();
    assert( headSize <= childSize );
    FOR(a,0,childSize)
    {
        LL u = headVec[a%headSize];
        LL v = childVec[a%childSize];
        printf("%lld %lld\n", u, v);
    }
}


void makeEdges(LL avoidIdx)
{
    FOR(a,0,cycleVec.size())
    {
        if ( avoidIdx == a )
        ...
        printEdge( cycleVec[avoidIdx], cycleVec[a] );
    }
}



void solveWithOneCycle()
{
    LL oneCycleIdx = getCycleIdx(1);
    assert(oneCycleIdx != -1);
    cout << "YES" << endl;;
    makeEdges(oneCycleIdx);
    exit(0);
}

void solveWithTwoCycle()
{
    LL twoCycleIdx = getCycleIdx(2);
    assert( twoCycleIdx != -1 );
    cout << "YES" << endl;
    cout << cycleVec[twoCycleIdx][0] << " " << cycleVec[twoCycleIdx][1] << endl;
    makeEdges(twoCycleIdx);
    exit(0);
}
...
//    freopen("input.txt", "r", stdin);
    ...
    else if ( getCycleIdx(2) != -1    &&   ( !isOddCycle() ) )
    {
        solveWithTwoCycle();
    }
