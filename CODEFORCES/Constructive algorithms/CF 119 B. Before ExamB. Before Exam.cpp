...
LL minProfSum = INT_MAX;
LL maxProfSum = INT_MIN;
...
void handleNotTaken()
{
    ...
    LL profSum = 0;
    FOR(a,0,thmCntInCard)
    {
        profSum += notTaken[a];
    }
    aMin(minProfSum, profSum);
    aMax(maxProfSum, profSum);
}
...
//    freopen("input.txt", "r", stdin);
    ...
    FOR(a,0,q)
    {
        VL thmInCard;
        LL profSum = 0;
        FOR(b,0,thmCntInCard)
        {
            LL thm;
            cin >> thm;
            thmInCard.push_back( thm );
            takenAr[thm] = 1;
            profSum += aAr[thm];
        }
        SORT(thmInCard);
        ques.insert( thmInCard );
        aMin(minProfSum, profSum);
        aMax(maxProfSum, profSum);
    }
    ...
    FOR(a,1,1+n)
    {
        if ( ! takenAr[a] )
        {
            notTaken.push_back( aAr[a] );
        }
    }
    assert( notTaken.size() >= (n%k) );
    ...
    double minAns = (1.0 * minProfSum) / thmCntInCard;
    double maxAns = (1.0 * maxProfSum) / thmCntInCard;

    printf("%0.10lf %0.10lf", minAns, maxAns);
