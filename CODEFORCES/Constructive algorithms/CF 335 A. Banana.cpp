...
string getSheet(LL sheetCnt)
{
    string sheet;
    FOR(a,'a','z'+1)
    {
        LL charNeeded = ceil( (1.0 * charCnt[a] ) / sheetCnt );
        FOR(b,0,charNeeded)
        {
            sheet.push_back( (char)a );
        }
    }
    while(sheet.size() < sheetLen)
    {
        sheet.push_back( 'a' );
    }
    return sheet;
...
bool isPos( LL sheetCnt )
{
    string sheet = getSheet(sheetCnt);
    return sheet.size() <= sheetLen;
}


LL getMinSheetCntPos( LL minVal, LL maxVal )
{
    if ( minVal >= maxVal )
    {
        return minVal;
    }
    LL midVal = (minVal + maxVal) / 2;
    if ( isPos(midVal) )
    {
        maxVal = midVal;
    }
    ...
    return getMinSheetCntPos(minVal, maxVal);
}
...
int main()
{
    ...
    if ( distinctCharCnt > sheetLen )
    {
        cout << -1;
        return 0;
    }

    LL minSheetCnt = getMinSheetCntPos(1, INT_MAX);

    string sheet = getSheet(minSheetCnt);

    cout << minSheetCnt << endl;
    cout << sheet;
