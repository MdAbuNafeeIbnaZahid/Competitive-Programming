...
Matrix getCumSumMat( const Matrix &grid )
{
    ...
    FOR(a,1,grid.size())
    {
        FOR(b,1,grid[a].size())
        {
            cumSum[a][b] = cumSum[a-1][b] + cumSum[a][b-1] + grid[a][b] - cumSum[a-1][b-1];
        }
    }
    return cumSum;
}

LL getRectSum(LL u, LL v, LL x, LL y, const Matrix &cumSum)
{
    if(u>x) { swap(u,x); }
    if(v>y) { swap(v,y); }
    LL rectSum = cumSum[x][y] - cumSum[u-1][y] - cumSum[x][v-1] + cumSum[u-1][v-1];
    return rectSum;
}

bool isPos( LL u, LL v, LL x, LL y, const Matrix &cumSum )
{
//    DBG4(u, v, x, y);
    LL len = abs(u-x) + abs(v-y) + 2;
//    DBG(len);

    LL bSum = getRectSum(u, v, x, v, cumSum) + getRectSum(x, v, x, y, cumSum);
//    DBG(bSum);
    if ( bSum == len )
    {
        return true;
    ...
    bSum = getRectSum(u, v, u, y, cumSum) + getRectSum(u, y, x, y, cumSum);
//    DBG(bSum);
    if ( bSum == len )
    {
        return true;
...
bool solve(const Matrix &grid)
{
    LL siz = grid.size();
    Matrix cumSumMat = getCumSumMat(grid);
    DBG( cumSumMat );
    FOR(a,0,siz)
    {
        FOR(b,0,siz)
        {
            FOR(c,0,siz)
            {
                FOR(d,0,siz)
                {
                    ...
                    if ( !isPos(a, b, c, d, cumSumMat) )
                    {
                        DBG4(a, b, c, d);
                        isPos(a, b, c, d, cumSumMat);
                        return false;
                    }
                }
            }
