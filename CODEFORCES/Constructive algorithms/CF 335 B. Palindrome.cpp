...
vector<char> firAns, lastAns;

void printReqTimes(char ch)
{
    FOR(a,0,REQ_ANS)
    {
        cout << ch;
...
void printCharVec( const vector<char> &v )
{
    for (auto it : v)
    {
        cout << it;
    }
//    FOR(a,0, min( (LL)v.size(), uptoIdx+1) )
//    {
//        cout << v[a];
//    }
}

//void printFromFirstAndLast()
//{
//    REV(lastAns);
//    if ( firAns.size() + lastAns.size() <= REQ_ANS )
//    {
//        printCharVec()
//    }
//}
...
void printAns(LL si, LL ei)
{
    while(si < ei)
    {
        ...
        if ( nSi==si+1  && nEi==ei-1 )
        {
            firAns.push_back( str[si] );
            lastAns.push_back( str[ei] );
        }
...
LL getDp(LL si, LL ei)
{
    ...
    if ( ret == -1 )
    {
        ...
        else if ( getDp(si+1, ei) > getDp(si, ei-1) )
        {
            nSi = si+1;
            nEi = ei;
            ret = getDp(nSi, nEi);
