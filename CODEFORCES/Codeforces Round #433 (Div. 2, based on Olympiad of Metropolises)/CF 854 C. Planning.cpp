...
#define SIZE 300009
LL n, k;
LL cAr[SIZE], ansAr[SIZE];

set<LL> avSet;
set<PairLL> flightSet;
LL totCost = 0;
...
//    freopen("input.txt", "r", stdin);
    ...
    {
        ILL( cAr[a] );
        flightSet.insert( MP( cAr[a], a ) );
    }
    FOR(a,k+1,k+n+1)
    {
        avSet.insert( a );
    }
    while( flightSet.size() )
    {
        set<PairLL>::iterator pairIt = flightSet.end();
        pairIt--;

        LL idx = pairIt->second;
        LL cost = pairIt->first;

        flightSet.erase( pairIt );

        set<LL>::iterator it = avSet.lower_bound( idx );
        totCost += ( *it - idx ) * cost;
        ansAr[ idx ] = *it;
