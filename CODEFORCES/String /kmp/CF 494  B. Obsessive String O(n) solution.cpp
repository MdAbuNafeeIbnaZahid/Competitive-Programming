...
LL dpAsWish[2*SIZE];
LL cumSumAsWish[2*SIZE];

void initDp(LL totalLen)
{
    dpAsWish[totalLen+1] = 1;
    cumSumAsWish[totalLen+1] = 1;
}

void processDp(LL i, LL patternLen, LL totalLen, LL patStartIdx)
{
    dpAsWish[i] = dpAsWish[i+1];
//    cumSumAsWish[i] = cumSumAsWish[i+1];
//    auto it = lower_bound( matchVec.begin(), matchVec.end(), i );
//    if ( it!=matchVec.end() && *it+patternLen-1 <= totalLen )
//    {
    if (patStartIdx<=totalLen)
    {
        dpAsWish[i] = ( dpAsWish[i] + cumSumAsWish[patStartIdx+patternLen] ) % MOD;
    }
//    }
    cumSumAsWish[i] = ( cumSumAsWish[i+1] + dpAsWish[i] ) % MOD;
}


LL calcAns( LL totalLen, const VL &matchVec, LL patternLen )
{
    LL a, b, c, d, e;
    LL ans = 0;
    FOR(a,1,totalLen+1)
    {
        auto it = lower_bound( matchVec.begin(), matchVec.end(), a );
        if ( it!=matchVec.end() && *it+patternLen-1 <= totalLen )
        {
            ans = ( ans + cumSumAsWish[*it+patternLen] ) % MOD;
        }
    }
    return ans;
}
...
VL getPrefixVec( string pattern )
{
    LL a, b, c, d, e;
    VL prefixVec;
    LL patternLen = pattern.size();
    LL j = 0;
    if ( patternLen > 0 )
    {
        prefixVec.push_back(j);
    }
    for (a = 1; a < patternLen; )
    {
...
VL getMatchIndxVec( string text, string pattern )
{
    LL a, b, c, d, e;
    LL patternLen = pattern.size();
    VL matchedIndexVec;
    string mergedString = pattern + "#" + text;
    VL failureVec = getPrefixVec( mergedString );
    DBG( failureVec );
    FOR(a,0,failureVec.size())
    {
        assert( failureVec[a] <= patternLen );
        if ( failureVec[a]==patternLen )
        {
            matchedIndexVec.push_back( a-2*patternLen+1 );
        }
    }
    return matchedIndexVec;
}


LL solve(string text, string pattern)
{
    LL a, b, c, d, e;
    VL matchIndexVec = getMatchIndxVec( text, pattern );
    LL patternLen = pattern.size();
    LL totalLen = text.size();
    initDp(totalLen);
//    matchIndexVec.push_back( SIZE );
    LL cur = matchIndexVec.size()-1;
    matchIndexVec.push_back( totalLen+9 );
    DBG(matchIndexVec);
    ROF(a,totalLen,0)
    {
        while(cur>=0 && a<= matchIndexVec[cur])
        {
            cur--;
        }
        cur++;
        processDp(a, patternLen, totalLen, matchIndexVec[cur]);
    }
    DBGAR(dpAsWish, 1, totalLen+1);
    DBGAR(cumSumAsWish, 1, totalLen+1);
    LL ans = calcAns(totalLen, matchIndexVec, patternLen);
    return ans;
}
