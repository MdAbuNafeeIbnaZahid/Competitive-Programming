...
void processDp(LL i, const VL &matchVec, LL patternLen, LL totalLen)
{
...
//    cumSumAsWish[i] = cumSumAsWish[i+1];
    ...
    {
        dpAsWish[i] = ( dpAsWish[i] + cumSumAsWish[*it+patternLen] ) % MOD;
    }
...
LL solve(string text, string pattern)
{
    ...
    ROF(a,totalLen,0)
    {
        processDp(a, matchIndexVec, patternLen, totalLen);
    }
