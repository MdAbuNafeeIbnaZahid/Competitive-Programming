...
LL getMatchCnt( VL text, VL pattern )
{
    LL matchCnt = 0;
    LL a, b, c, d, e;
...
//    prVec( pattern );
    ...
    VL failVec = getFailVec(pattern);
//    cout << "printing failVec" << endl;
//    prVec( failVec );
    for (auto it : failVec)
    {
        if (it==patternLen)
...
VL getStepVec( VL inpVec )
{
    LL a, b, c, d;
    VL stepVec;
    FOR(a,1,inpVec.size())
    {
        LL step = inpVec[a]-inpVec[a-1];
        stepVec.push_back( step );
    }
    return stepVec;
}

LL solve( VL bearWall, VL elephantWall )
{
    LL a, b, c,d , e;
    LL ret = 0;

    if ( elephantWall.size()==1 )
    {
        return bearWall.size();
    }

    if (elephantWall.size()>bearWall.size())
    {
        return 0;
    }

    VL bearStep = getStepVec( bearWall );
    VL elephantStep = getStepVec( elephantWall );




    ret = getMatchCnt(bearStep, elephantStep);
    return ret;
}
...
//    freopen("output.txt", "w", stdout);
    ...
    VL bearWall = takeVecInput(n);
    VL elephantWall = takeVecInput(w);

    LL ans = solve(bearWall, elephantWall);
    cout << ans;
    return 0;
}
