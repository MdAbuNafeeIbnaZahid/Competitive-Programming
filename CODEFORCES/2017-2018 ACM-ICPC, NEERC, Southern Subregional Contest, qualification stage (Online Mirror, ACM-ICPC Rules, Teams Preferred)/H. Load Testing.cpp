...
LL n;
LL aAr[SIZE];
LL leftNeed[SIZE], leftMax[SIZE];
LL rightNeed[SIZE], rightMax[SIZE];
LL ans;
int main()
{
//    freopen("input.txt", "r", stdin);
    ...
    {
        ILL( aAr[a] );
        leftMax[a] = max(leftMax[a-1]+1, aAr[a]);
        leftNeed[a] = leftNeed[a-1] + leftMax[a]-aAr[a];
    }

//    DBG(n);
//    cout << "printing leftMax " << endl;
//    prAr(leftMax, 1, n);
//
//    cout << "printing leftNeed " << endl;
//    prAr(leftNeed, 1, n);

    ROF(a,n,0)
    {
        rightMax[a] = max( rightMax[a+1]+1, aAr[a] );
        rightNeed[a] = rightNeed[a+1] + rightMax[a]-aAr[a];
    }
    ans = LLMAX;
    FOR(a,1,1+n)
    {
        LL posAns = leftNeed[a-1] + max(leftMax[a], rightMax[a])-aAr[a] + rightNeed[a+1];
        aMin(ans, posAns);
    }
    cout << ans;
    return 0;
