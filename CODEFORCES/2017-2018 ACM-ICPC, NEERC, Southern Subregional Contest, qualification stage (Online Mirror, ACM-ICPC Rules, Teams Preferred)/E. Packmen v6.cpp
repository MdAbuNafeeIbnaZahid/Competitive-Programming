...
//    DBG(t);
    LL a, b, c, d;
    vector<LL> pVec = packVec;
    vector<LL> aVec = asVec;

//    cout << "printing pVec " << endl;
//    prVec( pVec );
...
//    cout << "entering while loop" << endl;
    while( pVec.size() )
    {
        LL remT = t;
//        DBG( remT );
        LL curP = pVec[ pVec.size()-1 ];

//        DBG(curP);

        pVec.pop_back();

        if ( aVec.size() && aVec[ aVec.size()-1 ]>curP  )
        {
//            cout << "there is something in the right " << endl;
            if (  aVec[aVec.size()-1]-curP > t )
            {
//                cout << "right is too far " << endl;
                return 0;
            }
//            remT -= ( aVec[aVec.size()-1]-curP );
//            remT -= ( aVec[aVec.size()-1]-curP );
            LL doubleMove = t-2*( aVec[aVec.size()-1]-curP);
            LL singleMove = ( t-( aVec[aVec.size()-1]-curP) ) /2;
            remT = max( doubleMove, singleMove );
//            DBG(remT);
            while( aVec.size() && aVec[aVec.size()-1]>curP )
            {
                aVec.pop_back();
            }
        ...
        while( aVec.size() && abs(aVec[aVec.size()-1]-curP) <= remT )
        {
            aVec.pop_back();
        }
    }
...
//    cout << " printing asVec " << endl;
//    prVec( asVec );
//
//    cout << "printing packVec " << endl;
//    prVec( packVec );

    LL ans = binSearch(1, SIZE*3);
    cout << ans;
    return 0;
}
