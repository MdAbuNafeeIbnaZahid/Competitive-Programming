...
LL howMuchFromRight( VL const &vec, LL curHow )
{
    LL len = vec.size();
    LL curPos = len-1-curHow;
    LL ret = curHow;
    while( curPos>=0 && vec[curPos]==1 )
    {
        curPos--;
        ret++;
    }
...
VL solve( VL inp )
{
    ...
    for (auto it : inp)
    {
        ...
        rowVec[it] = 1;
        putCnt++;
        curConsOne = howMuchFromRight(rowVec, curConsOne);
        LL ans = 1 + putCnt - curConsOne;
        ret.push_back( ans );
    }
    return ret;
}
