...
double icpcAnsAr[SIZE][SIZE];
double otherAnsAr[SIZE][SIZE];

double getIcpc(LL seat, LL ic, LL ot)
{
    if (ic>icpc || ot>other)
    {
        return 0;
    ...
    if ( seat <= 0 )
    {
        icpcAnsAr[icpc-ic][other-ot] = 0;
        return 0;
    }
    double myChance = 2.0 / ( 2.0 + (ic<<1LL) + ot );
    double icChance = (ic<<1LL) / ( 2.0 + (ic<<1LL) + ot );
    double otChance = ot / ( 2.0 + (ic<<1LL) + ot );

    double ret = myChance;
    if ( icChance>eps )
    {
        ret += icChance * icpcAnsAr[icpc-ic+1][other-ot];
    }
    if ( otChance > eps )
    {
        ret += otChance * icpcAnsAr[icpc-ic][other-ot+1];
    }
    icpcAnsAr[icpc-ic][other-ot] = ret;
    return ret;
}


double getOther(LL seat, LL ic, LL ot)
{
    if (ic>icpc || ot>other)
    {
        return 0;
    ...
    if ( seat <= 0 )
    {
        otherAnsAr[icpc-ic][other-ot] = 0;
        return 0;
    }
    double myChance = 1.0 / ( 1.0 + (ic<<1LL) + ot );
    double icChance = (ic<<1LL) / ( 1.0 + (ic<<1LL) + ot );
    double otChance = ot / ( 1.0 + (ic<<1LL) + ot );

    double ret = myChance;
    if ( icChance>eps )
    {
        ret += icChance * otherAnsAr[icpc-ic+1][other-ot];
    }
    if ( otChance > eps )
    {
        ret += otChance * otherAnsAr[icpc-ic][other-ot+1];
    }
    otherAnsAr[icpc-ic][other-ot] = ret;
    return ret;
}
...
//        DBG3(n, icpc, other);
        // seat remaining
        FOR(a,max(1LL, n-icpc-other),1+n)
        {
            LL seatConsumed =  n-a;
            LL minIdx = max(seatConsumed-other,0LL);
            LL maxIdx = min(seatConsumed, icpc);
            FOR(b, minIdx, maxIdx+1LL )
            {
                getIcpc(a, icpc-b, other-seatConsumed+b);
                getOther(a, icpc-b, other-seatConsumed+b);
            }
        }
        double icpcAns = getIcpc(n, icpc, other);
        printf("%0.12lf\n", icpcAns);
        double otherAns = getOther(n, icpc, other);
        printf("%0.12lf\n", otherAns);
    }
    return 0;
}
