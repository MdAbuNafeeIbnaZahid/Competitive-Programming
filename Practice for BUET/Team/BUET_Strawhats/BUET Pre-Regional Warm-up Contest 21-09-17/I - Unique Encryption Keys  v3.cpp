...
LL nextAr[SIZE], kAr[SIZE], ansAr[SIZE];
LL okAr[SIZE];

// bi, ei, i
set< TripleLL > setTrip;
map<LL, LL> prevMap;
LL M, Q;


void prOS( ordered_set toPr )
{
    LL a, b, c;
    LL siz = toPr.size();
    FOR(a,0,siz)
    {
        cout << "[" << a << "] = " << *(toPr.find_by_order(a) ) << endl;
    }
}
...
//            prMap( prevMap );
//
//            cout << "printing nextAr " << endl;
//            prAr(nextAr, 1, M);


            if ( prevMap[ kAr[a] ]==0 ) // it is unique
            {
...
//        prOS( OS );
        ...
        FOR(a,1,1+Q)
        {
            LL bi, ei;
            ILL2(bi, ei);
            setTrip.insert( TripleLL(bi, ei, a) );
        }


//        cout << "printing setTrip " << endl;
//        prSet( setTrip );
...
//                cout  << "will handle it now " << endl;

                setTrip.erase( setTrip.begin() );
                LL bi = curQ.first;
                LL ei = curQ.second;
                LL idx = curQ.third;
...
//                DBG3(bi, ei, idx);
                ...
                LL ans = (OS.order_of_key(ei+1) )-(OS.order_of_key(bi) );
...
//                DBG(ans);
                ...
                {
                    okAr[ idx ] = 0;
                    LL ansIdx = *( OS.lower_bound( a ) );

//                    DBG( ansIdx );

                    ansAr[idx] = kAr[ ansIdx ];
                }
            }
            OS.erase( nextAr[a] );
        }
