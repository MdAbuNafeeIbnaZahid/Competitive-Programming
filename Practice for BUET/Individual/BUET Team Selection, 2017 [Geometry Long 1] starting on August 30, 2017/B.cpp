...
double calc(double xx, double yy, double d)
{
    if ( xx<yy+eps )
    {
        swap(xx, yy);
    }
    double p = mySqrt(xx*xx - d*d);
    double q = mySqrt(yy*yy - d*d);
    double b = d/(1+p/q);
    double c = p*b/d;
    return c;
}

double binSearch(double minVal, double maxVal)
{
    ...
    {
        return binSearch(midVal, maxVal);
    }
}
...
//    freopen("input.txt", "r", stdin);
    ...
    FOR(i,1,1+T)
    {
        ...
        double ans = binSearch(0, min(x,y) );
