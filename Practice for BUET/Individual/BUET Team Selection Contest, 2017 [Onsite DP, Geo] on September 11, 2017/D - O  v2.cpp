...
LL lisAr[SIZE], revLisAr[SIZE];


void makeLisAr(vector<LL> giVec, LL *ar)
{
    LL a, b, c, d,e , f;
    ordered_set os;
    FOR(a,0,giVec.size())
    {
        LL val = os.order_of_key( giVec[a] );
        ar[ a ] = val + 1;
        if ( os.find_by_order( val ) != os.end() )
        {
            LL delVal = *os.find_by_order( val );
            os.erase( delVal );
        }
        os.insert( giVec[a] );
    }
...
//    freopen("input.txt", "r", stdin);
    ...
    while( scanf("%lld", &N) != EOF )
    {
        ...
        makeLisAr(vecLL, lisAr);
        REV( vecLL );
        makeLisAr(vecLL, revLisAr);
        LL ans = 1;
        FOR(a,0,N)
        {
            aMax(ans, 2*min(lisAr[a], revLisAr[N-1-a])-1 );
        }
        cout << ans << endl;
    }
    return 0;
