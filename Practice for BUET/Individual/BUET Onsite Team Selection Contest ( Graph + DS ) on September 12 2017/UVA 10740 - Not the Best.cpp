...
void resizeMS( multiset<LL> &ms )
{
    while( ms.size() > SET_SIZE )
    {
        multiset<LL>::iterator it = ms.end();
        it--;
        ms.erase( it );
...
void bfs(LL s)
{
    ...
    while( Q.size() )
    {
        ...
        for (auto it : gr[u] )
        {
            LL nextDis = w + it.second;
            LL nextNode = it.first;
            if ( disAr[nextNode].size()<SET_SIZE ||
                  getMax( disAr[nextNode] )>nextDis  )
            {
                disAr[nextNode].insert( nextDis );
                Q.push( PairLL(nextNode, nextDis) );
                resizeMS( disAr[nextNode] );
            }
        }
    }
}
...
//    freopen("input.txt", "r", stdin);
    ...
    while(1)
    {
        ...
        {
            multiset<LL>::iterator it = disAr[y].begin();
            FOR(a,0,k-1)
            {
                it++;
            }
