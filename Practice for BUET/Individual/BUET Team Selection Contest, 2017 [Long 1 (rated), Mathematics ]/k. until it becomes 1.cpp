...
#define SIZE 1000009
LL T, N;
LL sivAr[SIZE], primeUpto[SIZE];
double eAr[SIZE];
vector<LL> primeFacAr[SIZE];

void makeSivAndPrimeFacAr()
{
    LL a, b, c, d, e, f;
    FOR(a,2,SIZE)
    {
    ...
    FOR(a,2,SIZE)
    {
        if ( sivAr[a] == a )
        {
            ...
            {
                sivAr[ b ] = a;
                primeFacAr[ b ].push_back( a );
            }
        }
        primeUpto[a] = primeUpto[a-1] + (sivAr[a]==a);
    }
}
...
void makeEar()
{
    ...
    FOR(a,2,SIZE)
    {
        LL x = primeFacAr[a].size(); // divides
        LL y = primeUpto[a] - x; // NOT divide

        eAr[a] = 1 + ( (1.0* y) /x );
        double smallSum = 0;
        for ( auto it : primeFacAr[a] )
        {
            smallSum += eAr[ a/it ];
        }
        eAr[a] += ( smallSum / x );
