...
LL fakeAr[SIZE], realAr[SIZE], SCCedgeCnt[SIZE];
...
//            DBG2(a, dfu.getCount(a));
            if ( SCCedgeCnt[dfu.getParent(a)]>0 && SCCout[ dfu.getParent(a) ].size()==0 && ansAr[a]==UNDEFINED )
            {
                ansAr[a] = INFIN;
            }
        }
...
//        printGausAr( fakeCnt );
        ...
        FOR(a,1,1+q)
        {
            ...
            else
            {
                ...
                else if ( abs(ansAr[querNode] ) <= eps )
                {
                    printf("0.000\n");
                }
