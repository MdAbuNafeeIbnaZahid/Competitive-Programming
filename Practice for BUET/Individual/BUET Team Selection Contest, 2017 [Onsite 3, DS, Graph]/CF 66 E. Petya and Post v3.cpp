...
/******   END OF HEADER *********/
#define SIZE 100009
LL n, aAr[SIZE], bAr[SIZE];
LL difAr[SIZE], cumSumAr[SIZE];


set<LL> ansSet;

void calcAnsOneWay(LL ifOp = 0)
{
//    vector<LL> ret;
    LL a, b, c, d, e;
    LL minCumSum = INT_MAX;
    FOR(a,0,n)
    {
        difAr[a] = aAr[a] - bAr[a];
        if ( a==0 )
        {
            cumSumAr[a] = difAr[a];
            minCumSum = min(minCumSum, cumSumAr[a]);
            continue;
        }
        cumSumAr[a] = cumSumAr[a-1] + difAr[a];
        minCumSum = min(minCumSum, cumSumAr[a]);
    }

//    cout << "difAr " << endl;
//    printLLArray(difAr, 0, n-1);
//    cout << "cumSumAr" << endl;
//    printLLArray( cumSumAr, 0, n-1 );
//
//    DBG( minCumSum );
...
//    cout << "transformed aAr and bAr for reverse calc" << endl;
//    cout << "aAr " << endl;
//    printLLArray(aAr, 0, n-1);
//    cout << "bAr " << endl;
//    printLLArray( bAr, 0, n-1 );
