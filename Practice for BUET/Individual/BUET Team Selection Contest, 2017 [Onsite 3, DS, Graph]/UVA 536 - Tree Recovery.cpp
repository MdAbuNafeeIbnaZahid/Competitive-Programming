...
#define MID 2
#define RIGHT 3
char pre[SIZE], in[SIZE], post[SIZE];

void handlePrint(char *preStr, LL psi, LL pei, char *inStr, LL isi, LL iei)
{
    LL a, b, c, d, e;
    LL typAr[SIZE];
    RESET( typAr );
    ...
    FOR(a,0,len)
    {
        typAr[ inStr[a+isi]-'A' ] = LEFT;
        if ( inStr[a+isi] == preStr[psi] )
        {
            inMid = a+isi;
            typAr[ inStr[a+isi]-'A' ] = MID;
            FOR(b,a+1,len)
            {
                typAr[ inStr[b+isi]-'A' ] = RIGHT;
            }
            break;
        }
    }
    ...
    handlePrint(preStr, psi+1, psi+inMid-isi, inStr, isi, inMid-1);
    handlePrint(preStr, psi+inMid-isi+1, pei, inStr, inMid+1, iei);
    printf("%c", preStr[psi]);
}
...
//    freopen("output.txt", "w", stdout);
    while( scanf("%s", pre) != EOF )
    {
        scanf("%s", in);
        LL len = strlen( pre );
        handlePrint(pre, 0, len-1, in,  0, len-1);
        printf("\n");
    }
    return 0;
}
