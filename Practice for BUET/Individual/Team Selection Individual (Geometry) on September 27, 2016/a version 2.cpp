...
int main()
{
    ...
    for (a = 1; a <= N; a++)
    {
        ...
        CcA = straightLine(C, cA);
        AaC = straightLine(A, aC);
        E = interSecOfTwoLines(CcA, AaC);
        AaB = straightLine(A, aB);
        BbA = straightLine(B, bA);
        F = interSecOfTwoLines(AaB, BbA);
        printf("%0.8lf %0.8lf %0.8lf %0.8lf %0.8lf %0.8lf\n", D.x, D.y, E.x, E.y, F.x, F.y);
    }
    return 0;
}
