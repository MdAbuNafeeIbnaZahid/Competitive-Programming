...
#define SIZE 2000009
LL N, M;
LL ar[SIZE], sumAr[SIZE], dpAr[SIZE];
...
//        DBG(side);
        FOR(b,0,(1<<M))
        {
            FOR(c,0,M)
            {
                sumAr[b] += (checkBit(b, c) * ar[c] );
            }
            bitAr[ __builtin_popcountll(b) ].PUB( b );
        }
//        prAr(sumAr, 0, (1<<M));
        dpAr[0] = 1;
        FOR(b,0,29)
