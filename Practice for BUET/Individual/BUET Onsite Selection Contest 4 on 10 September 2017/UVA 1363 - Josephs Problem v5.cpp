...
//    prVec( vec );
    ...
    while( vec.size() > 1 )
    {
        LL ei = vec[ vec.size()-1 ].first;
        LL si = vec[ vec.size()-2 ].first + 1;
        vec.pop_back();
        aMin(ei, num);
        if (si>ei)
        {
        ...
        LL addee = ( (ei-si+1) * ( kk%si + kk%ei ) ) / 2;
        ret += addee;
    }
    if ( num > kk )
    {
