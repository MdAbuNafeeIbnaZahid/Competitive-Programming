...
LL ballAr[SIZE];
LL ans;
LL ballCumSumAr[SIZE], buckCntAr[SIZE];
LL bucketCumSumAr[SIZE];
...
LL solve()
{
    ...
    FOR(a,1,SIZE)
    {
        bucketCumSumAr[a] = bucketCumSumAr[a-1] + buckCntAr[a];
        ballCumSumAr[ a ] = ballCumSumAr[a-1] + buckCntAr[a]*a;
    }
    ...
    FOR(a,2,SIZE)
    {
        ...
        {
            LL posRet = 0;
            posRet = bucketCumSumAr[a] * a - ballCumSumAr[a];
            for (b = a<<1; b < SIZE; b+=a)
            {
                isPrime[b] = false;
                LL bc = bucketCumSumAr[b]-bucketCumSumAr[b-a];
                LL totTargetBall = bc * b;
                LL avBall = ballCumSumAr[b]-ballCumSumAr[b-a];
                LL addee = totTargetBall-avBall;
                posRet += addee;
            }
            aMin(ret, posRet);
        }
...
//    freopen("input.txt", "r", stdin);
    ...
    FOR(a,1,1+T)
    {
        ...
        {
            ILL( ballAr[b] );
            if ( ballAr[b] < ballAr[b-1] )
            {
                ans += (ballAr[b-1] - ballAr[b]);
                ballAr[b] = ballAr[b-1];
            }
            buckCntAr[ ballAr[b] ]++;
        }
