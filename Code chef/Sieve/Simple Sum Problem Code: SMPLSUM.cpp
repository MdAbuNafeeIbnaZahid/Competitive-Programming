...
/******   END OF HEADER *********/
//#define SIZE 10000009LL
#define SIZE 10000009LL

// lpfVec[0] = 0
// lpfVec[1] = 0
vector<long long> getLpfVec(long long upto)
{
    vector<long long> lpfVec(upto+1, 0);
    vector<long long> primeVec;
    for(long long a = 2; a < upto+1; a++)
    {
        if ( lpfVec[a]==0 )
        {
            lpfVec[a] = a;
        ...
        for (long long b = 0; b < primeVec.size(); b++ )
        {
            if (primeVec[b]>lpfVec[a] || primeVec[b]*a>upto )
            {
                break;
            }
            lpfVec[ primeVec[b]*a ] = primeVec[b];
        }
    }
    return lpfVec;
}

LL getAnsBrute(LL num)
{
    LL ret = 0;
    FOR(a,1,num+1)
    {
        ret += num/getGcd(num, a);
    }
    return ret;
}

LL getAnsEfe(LL num, const VL &lpfVec)
{
    LL ret = 1;
    while( num != 1 )
    {
        LL curLpf = lpfVec[num];
        num /= curLpf;
        LL curMul = curLpf * (curLpf-1) + 1;
        LL curLpfSq = curLpf*curLpf;
//        LL curPow = 1;
        DBG(curMul);
        while( lpfVec[num]==curLpf )
        {
            num /= curLpf;
            curMul = curMul * (curLpfSq) - curLpf + 1;
            DBG(curMul);
        }
