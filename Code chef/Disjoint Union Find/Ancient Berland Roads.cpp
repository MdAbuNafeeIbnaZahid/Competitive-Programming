...
#define SIZE 500009
int N, M, Q, D, K, P, A, X;
int i, j, a, b, x, y, ifDestroyed[SIZE], parent[SIZE],  maxPopu = -1, ans[SIZE];
long long sum[SIZE];
...
{
    //cout << "changeValue called with idx = " << idx << ", newValue = " << newValue << endl;
...
int main()
{
    ...
    for (i = 1; i <= M; i++)
    {
        if ( ifDestroyed[i]==0 )
        {
            //cout << "making union for " << i << "th road" << endl;
    ...
    for (i = Q; i >= 1; i--)
    {
        //cout << "Query = " << i << endl;
        //cout << "currentPopu ";
        //for (j = 1; j <= N; j++) cout << currentPopu[j] << " ";
        //cout << endl;
        //cout << "popuSet ";
        //printIntMulSet( popuSet );
        ans[i] = *(--popuSet.end());
