...
// to , idx
vector< PairLL > gr[SIZE];

// u, v, type
vector< TripleLL > edgVec;
LL visAr[SIZE], discoverAr[SIZE], lowAr[SIZE], parentAr[SIZE];
LL ti;


void findBridge(LL u)
{
//    cout << "in findBridge " << endl;
//    DBG(u);
    ti++;
    lowAr[u] = discoverAr[u] = ti;
    visAr[u] = 1;
//    vector<LL> childVec;
//    LL childCnt = 0;
    for (auto &it : gr[u])
...
//        cout  << "will handle it " << endl;
//        DBG( edgVec[it.second] );

        if ( edgVec[ it.second ].third==UNDEFINED )
        {
            if ( edgVec[ it.second ].first==u )
            {
                edgVec[ it.second ].third=FRONT;
            }
            else
            {
                edgVec[ it.second ].third=BACK;
            }
...
//        DBG( edgVec[it.second] );
        ...
        if ( !visAr[to])
        {
            parentAr[to] = u;
            findBridge(to);
            aMin(lowAr[u], lowAr[to]);

            if ( discoverAr[u] < lowAr[to] /* && parentAr[u]!=0 */ )
            {
                LL idx = it.second;
                edgVec[idx].third = BRIDGE;
            }

//            childCnt++;
//            childEd.push_back( it.second );
        }
...
//    freopen("output.txt", "w", stdout);
    ...
    while(1)
    {
        ...
        FOR(a,0,m)
        {
            LL u, v;
            ILL2(u, v);
            edgVec.push_back( TripleLL(u, v, UNDEFINED) );
            gr[u].PUB( PairLL(v, a) );
            gr[v].PUB( PairLL(u, a) );
        }
...
//
        ...
        for (auto it : edgVec)
        {
            if ( it.third == FRONT || it.third == UNDEFINED )
            {
                printf("%lld %lld\n", it.first, it.second);
            }
            else if ( it.third == BRIDGE )
            {
                printf("%lld %lld\n", it.first, it.second);
                printf("%lld %lld\n", it.second, it.first);
            }
