...
void changeAr(LL *ar, LL si, LL ei, LL change)
{

//    cout << "in changeAr " << endl;
//    DBG3(si, ei, change);
    LL a, b, c, d, e;
    LL segSize = ei-si+1;
//    DBG(segSize);
    LL len = n;
    LL newAr[len];
    memcpy(newAr+si+change, ar+si, segSize*sizeof(LL) );
//    cout << "copied the segment" << endl;
//    prAr(newAr, 0, n-1);
    LL  cur = 0;
    FOR(a,0,len)
    {
        ...
        while(cur>=si+change && cur<=ei+change)
        {
            cur++;
        }
        newAr[cur++] = ar[a];
    }
    memcpy(ar, newAr, n*sizeof(LL));
}


bool dfs(LL dep, LL prun, LL ar[])
{
//    cout << "in dfs " << endl;
//    DBG2(dep, prun);
    LL a, b, c, d, e, f;
...
//    DBG(h);
    LL fVal = dep + h;
//    DBG(fVal);
    if ( 3*dep + segCnt > 3*prun )
    {
        return false;
    }
//    cout << " will generate children " << endl;
    FOR(a,0,len)
    {
//        DBG(a);
        FOR(b,a,len)
...
//            DBG(b);
            LL segSize = b-a+1;
//            DBG(segSize);
            FOR(c,0,len-segSize+1)
            {
//                DBG(c);
                LL change = c-a;
//                DBG(change);
                LL newAr[len];
                memcpy(newAr, ar, len*sizeof(LL));
//                cout << "newAr is initiated " << endl;
//                prAr(newAr, 0, n-1);
                changeAr(newAr, a, b, change);
//                cout << "newAr is changed " << endl;
//                prAr(newAr, 0, n-1);
                LL newH = getSegCnt(newAr)/3;
                if ( getSegCnt(newAr) +3*dep+3 > 3*prun)
                {
                    continue;
                }
                bool res = dfs(dep+1, prun, newAr);
                if (res)
                {
//                    prAr(ar, 0, n-1);
                    return true;
