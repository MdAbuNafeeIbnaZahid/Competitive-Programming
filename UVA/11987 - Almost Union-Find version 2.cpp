...
int f2(int p, int q)
{
    ...
    for (i = 0; i < childs[ p ].size(); i++)
    {
        //cout << " childs[ p ].size() = " << childs[ p ].size() << endl;
        //cout << "childs[ p ][i] = " << childs[ p ][i] << endl;
        if (parent[ childs[ p ][i] ] == p) parent[ childs[ p ][i] ] = parent[p];
    }
    //cout << "after the for loop in f2" << endl;


    parent[p] = myFindParent(q);
    for (i = 0; i<childs[parent[p]].size(); i++) if ( childs[parent[p]][i] == p ) break;
    if (i >= childs[parent[p]].size()) childs[ parent[p] ].push_back( p );
