...
LL findPeak(LL si, LL ei, LL *ar)
{
    ...
    if ( ar[mid] > ar[mid-1] )
    {
        return findPeak( mid, ei, ar );
    }
    else
    {
        return findPeak(si, mid-1, ar);
    }
}


LL findInc(LL si, LL ei, LL val, LL *ar)
{
    LL mid = (si+ei)/2;
    if ( si>ei )
    {
    ...
    {
        return findInc(mid+1, ei, val, ar);
    }
    else if ( val < ar[mid] )
    {
        return findInc( si, mid-1, val, ar );
    }
}

LL findDec(LL si, LL ei, LL val, LL *ar)
{
    LL mid = (si+ei)/2;
    if ( si > ei )
    {
    ...
    {
        return findDec( mid+1, ei, val, ar );
    }
    else if ( val > ar[mid] )
    {
        return findDec(si, mid-1, val, ar);
    }
}
...
int main()
{
    ...
    FOR(a,1,1+kase)
    {
        ...
        aAr[N+1] = INT_MIN;
        LL peakIdx = findPeak(1, N, aAr);
        LL ans = findInc( 1, peakIdx, X, aAr );
        if (ans == -1)
        {
            ans = findDec(peakIdx, N, X, aAr);
        }
