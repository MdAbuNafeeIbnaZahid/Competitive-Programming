...
int main()
{
    ...
    for (a = 1; a <= T; a++)
    {
        ...
        for (b = 0; b < N; b++)
        {
            posX = gr[SOURCE].size();
            posY = gr[tail(b)].size();
            gr[SOURCE].push_back( edge(SOURCE, tail(b), icePieceArray[b].ni, posY ) );
            gr[tail(b)].push_back( edge(tail(b), SOURCE, 0, posX) );

            posX = gr[tail(b)].size();
            posY = gr[head(b)].size();
            gr[tail(b)].push_back( edge(tail(b), head(b), icePieceArray[b].mi, posY) );
            gr[head(b)].push_back( edge(head(b), tail(b), 0, posX) );
        }
        for (b = 0; b < N; b++)
        {
            for (c = b+1; c < N; c++ )
            {
                if ( isReachable(icePieceArray[b], icePieceArray[c]) )
                {
                    posX = gr[head(b)].size();
                    posY = gr[tail(c)].size();
                    gr[head(b)].push_back( edge( head(b), tail(c), INT_MAX, posY ) );
                    gr[tail(c)].push_back( edge(tail(c), head(b), 0, posX ) );

                    posX = gr[head(c)].size();
                    posY = gr[tail(b)].size();
                    gr[head(c)].push_back( edge(head(c), tail(b), INT_MAX, posY) );
                    gr[tail(b)].push_back( edge(tail(b), head(c), 0, posX ) );
                }
            }
        }
        printGraph();
